...
from luminolex.config import (
    MODEL_NAME, ADAPTER_REPO, DISPLAY_MODEL,
    BRAND, BRAND_DIM, INFO, SUCCESS, TEXT_DIM, TEXT_MAIN,
)
...
def load(verbose: bool = True):
    ...
    if verbose:
        console.print(Rule(f"[bold {BRAND}] Loading LuminoLex [/]", style=BRAND_DIM))
        console.print()
    ...
    with LoadingStep(f"Attaching LoRA adapter ({DISPLAY_MODEL}) …"):
        try:
            ...
            model = PeftModel.from_pretrained(
                base_model, ADAPTER_REPO, device_map=None, is_trainable=False
            )
            if has_cuda:
                model = model.cuda()
